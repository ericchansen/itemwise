...
- **Cherry-pick rebase for feature branch reconstruction (2026-03-02):** After a master rollback, reconstructed `feat/image-analysis-v2` by branching off `fix/auth-flash-clean` and cherry-picking 6 commits from the original `feat/image-analysis` branch (skipping `docs(squad)` commits). Key gotcha: commits that bundle untracked squad-template files require `git clean -fd` on those paths before cherry-pick will proceed. Stash any local uncommitted changes before cherry-picking commits that touch the same files. This gives a clean linear history: master → auth flash fix → image analysis work.
